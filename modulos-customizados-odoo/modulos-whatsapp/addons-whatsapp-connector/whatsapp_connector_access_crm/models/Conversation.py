...
GROUP_TEAM = 'whatsapp_connector_access_crm.chat_access_crm_team'
GROUP_EMPTY = 'whatsapp_connector_access_crm.chat_access_crm_empty'


class Conversation(models.Model):
    _inherit = 'acrux.chat.conversation'

    access_team_id = fields.Many2one('crm.team', string='CRM Access',
    ...
    def get_access_agent(self, args):
        ...
        if not flag and (is_team or is_empty):
            out = []
            if is_team:
                team_ids = self.env.user.chatroom_access_crm_team_ids.ids
                if team_ids:
                    out = [('access_team_id', 'in', team_ids)]
                else:
                    out = [(0, '=', 1)]
            else:
                out = [('access_team_id', '=', False)]
            if ret:
    ...
    def parse_notification(self, datas):
        ...
        if flag:
            ...
            datas = self.acrux_access_field_empty(datas, users, self.access_team_id, GROUP_EMPTY, GROUP_TEAM)
            member_ids = self.access_team_id.chatroom_user_ids.ids
            datas = self.acrux_access_member_ids(datas, users, member_ids, GROUP_TEAM)

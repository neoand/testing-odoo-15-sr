...
class AcruxChatConnector(models.Model):
    ...
    is_connection_chatsmart = fields.Boolean('Direct Connection Chat Smart', default=False)

    endpoint = fields.Char('API Endpoint', required=True, default='https://api.chat-smart.io/', invisible=False,
    ...
    def Xresponse_handler(self, req):
        '''
            :overide
        '''
        self.ensure_one()
    ...
    def ca_request(self, path, data={}, params={}, timeout=False, ignore_exception=False):
        ...
        if self.apichat_is_direct():
            if path == 'send':
                ...
                data['type'] = str.lower( data['type'].title())
    ...
    def apichat_is_direct(self):
        self.ensure_one()
        return self.is_connection_chatsmart and self.connector_type == 'apichat.io'
    ...
    def apichat_get_status(self):
        self.ensure_one()
        _logger.debug("++++++++++++++++++apichat_get_status+++++++++++++++++")
    ...
    def apichat_set_settings(self):
        ...
        self.ensure_one()
        data = {
            'webhook': '%s/chatsmart/whatsapp_connector/%s' % (self.odoo_url.rstrip('/'), self.uuid),
            'notify_ack': False,
    ...
    def init_free_test(self):
        _logger.debug("+++++++++++++++++++init_free_test++++++++++++++++++++++")
        self.ensure_one()
        data = self._get_custom_info()
        self.uuid = random.randrange(100000, 1000000)
        self.endpoint = 'https://api.chat-smart.io/connector/whatsapp_web/'  # backwards compatibility
        data.update({
            'tz': self.env.user.tz , 
            'connector_name': self.name,
            'webhook_url': '%s/chatsmart/whatsapp_connector/%s' % (self.odoo_url.rstrip('/'), self.uuid),
            })
        result = self.ca_request('account_new', data)        

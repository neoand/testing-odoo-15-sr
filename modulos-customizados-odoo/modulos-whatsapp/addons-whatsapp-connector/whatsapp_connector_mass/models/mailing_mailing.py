...
class Mailing(models.Model):
    ...
    spam_level = fields.Selection([('1', '600 messages per hour'),
                                   ...
                                   ('3', '100 messages per hour'),
                                   ('max', 'Max. speed')], 'Speed', default='1',
    ...
    @api.depends('ws_template_id')
    def _compute_extra_text(self):
        for record in self:
            extra_text = []
            if record.ws_template_id and record.ws_template_id.button_ids:
                extra_text = '\n'.join(map(lambda btn: '[ %s=%s ]' % (_('button'), btn),
                                           record.ws_template_id.button_ids.mapped('text')))
            if record.ws_template_id and record.ws_template_id.chat_list_id:
                extra_text.append('[ %s=%s ]' % (_('List'), record.ws_template_id.chat_list_id.name))
            record.extra_text = '\n'.join(extra_text) if extra_text else False
    ...
    def _constrain_enable_hour(self):
        for r in self:
            ...
            if r.enable_from_hour or r.enable_to_hour:
                ...
                if r.enable_from_hour < 0.0 or r.enable_to_hour < 0.0 or r.enable_from_hour > 24.0 or \
                        r.enable_to_hour > 24.0:
    ...
    def ws_validate_supported_models(self):
        ...
        if self.mailing_model_real not in ['crm.lead', 'event.registration',
                                           ...
                                           'acrux.chat.conversation', 'sale.subscription', 'helpdesk.ticket']:
    ...
    def generate_ws_message_contacts(self):
        def get_chat(mailing, partner_id, record=False):
            conn_id = mailing.connector_id
            if record and hasattr(record, 'conversation_id') and record.conversation_id.connector_id.id == conn_id.id:
                conv_ids = record.conversation_id
            else:
        ...
        for record in records:
            if mailing.mailing_model_real in ['event.registration', 'hr.applicant',
                ...
                conv_id = get_chat(mailing, partner_id, record)
                contacts.append({'number': conv_id.number or partner_id.mobile or partner_id.phone,
                                 'name': partner_id.name, 'doc_name': record.name, 'record': record,
                                 'conv_id': conv_id})
            elif mailing.mailing_model_real == 'helpdesk.ticket':
                partner_id = record.partner_id
                conv_id = get_chat(mailing, partner_id, record)
                name = record.partner_name or partner_id.name or ''
                number = conv_id.number or partner_id.mobile or partner_id.phone or record.partner_phone or ''
                ...
                for contact in contact_ids:
                    contacts.append({'number': contact.ws_phone_normalized,
                                     ...
                                     'record': contact,
                                     'conv_id': conv_id})
            elif mailing.mailing_model_real == 'acrux.chat.conversation':
                if record.connector_id.id == mailing.connector_id.id:
                    conv_id = record
                else:
                    conv_id = self.env['acrux.chat.conversation']
                contacts.append({'number': record.number, 'name': record.name,
                                 'doc_name': record.name, 'record': record, 'conv_id': conv_id})
            else:
                contacts = self.generate_ws_message_hook(contacts, record)
        return contacts

    def generate_ws_message_hook(self, contacts, record):
        return contacts
    ...
    @api.model
    def _cron_check_numbers(self, minutes, connector_id=False, pack=1, cron_list=[]):
        ...
        for mailing in mailing_ids:
            ...
            if ret.get('result') == 'all good':
                mailing.state = 'sending'
                # SEPARATE
                if cron_list and mailing.connector_type == 'gupshup':
                    Message = self.env['acrux.chat.message'].with_context(active_test=False)
                    domain_search = [('mailing_id', '=', mailing.id),
                                     ('cron_id', '=', False)]
                    mess_ids = Message.search(domain_search, order='id')
                    if mess_ids and len(mess_ids) > 100:
                        cron_pos = 0
                        n = int(len(mess_ids) / len(cron_list)) + 1
                        part = range(0, len(mess_ids), n)
                        for i in part:
                            range_ids = mess_ids[i:i + n]
                            range_ids.write({'cron_id': cron_list[cron_pos]})
                            cron_pos += 1
    ...
    @api.model
    def _cron_send_ws(self, minutes, connector_id=False, cron_id=False):
        duration = minutes * 60 * 0.8
        max_time = time() + duration
        for state in ['sending']:
            _logger.info('*** _cron_send_ws start: %s (Connector: %s - Cron: %s)' % (state, connector_id, cron_id))
            data = []
            if connector_id:
            ...
            if mailing and mailing.connector_id:
                ...
                if mailing.connector_status:
                    ...
                    if mailing.enable_from_hour <= now_float <= mailing.enable_to_hour:
                        mailing.action_send_ws(duration, max_time, cron_id)
                    else:
    ...
    def action_send_ws(self, duration, max_time, cron_id=False):
        '''
        ...
        domain_search = [('mailing_id', '=', self.id),
                         ...
                         ('error_msg', '=', False)]
        # - message_sent_count puede superar message_limit si hay varios cron.
        # - En _cron_check_numbers se debe establecer que se separan los mensajes,
        #   y para cada uno se debe crear cada _cron_send_ws.
        # - Asignar connector_id para cada conector en todos los cron, si usa distintos conectores.
        # - Se divide en varios cron solo si hay más de 100 mensajes.
        messages_draft = Message.search(domain_search + [('cron_id', '=', cron_id)], limit=message_limit, order='id')
        ...
        if messages_draft:
            ...
            new_messages_draft = Message.search_count(domain_search)
            if new_messages_draft == 0 and Message.search([('mailing_id', '=', self.id)], limit=1):
                set_done = True
        if set_done:
    ...
    def build_ws_messages(self, contacts):
        ...
        for contact in contacts:
            ...
            if record._name == 'acrux.chat.conversation' \
                    and record.connector_id.connector_type not in ['apichat.io', 'gupshup']:
                self.create_trace_error(contact['number'], contact['record'], 'Invalid Connector Type')
                continue
            if not valid:
            ...
            if self.check_unique_contact and number_cleaned in unique_contact:
                ...
                if self.ws_template_id:
                    ...
                    messages_dict = self._build_ws_parse_buttons(messages_dict)
                ...
                if not conv_id:
                    conv_id = Conversation.create({'connector_id': self.connector_id.id,
                                                   ...
                                                   'name': contact['name'],
                                                   'status': 'done'})
                self.build_ws_messages_conv_hook(conv_id, contact)
    ...
    def build_ws_messages_conv_hook(self, conv_id, contact):
        pass
    ...
    def _build_ws_parse_buttons(self, msg_datas):
        if msg_datas:
            if self.ws_template_id.button_ids:
                msg_datas[-1]['button_ids'] = self.ws_template_id.button_ids.get_to_create()
            elif self.ws_template_id.chat_list_id:
                msg_datas[-1]['chat_list_id'] = self.ws_template_id.chat_list_id.copy().id
        return msg_datas
    ...
    def _build_ws_message_template(self, res_id):
        ...
        if self.ws_template_id.waba_template_id:
            ...
            ttype = self.ws_template_id.waba_template_id.template_type.lower()
            if ttype in ['document', 'image', 'video'] and self.ws_template_id.waba_template_id.container_meta:
                try:
                    template_data = json.loads(self.ws_template_id.waba_template_id.container_meta)
                    template_message = {'type': ttype}
                    template_message[ttype] = {'id': template_data['mediaId']}
                    msg_base['template_data'] = json.dumps(template_message)
                    attachment_ids = []  # not attach
                except Exception as e:
                    print(e)
        if values.get('body'):

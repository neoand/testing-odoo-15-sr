...
class ExcelData(http.Controller):
    ...
    def download_excel_data(self, template_id, access_token):
        ...
        records = request.env[odc_template.model].sudo(odc_template.user_id.id).with_context(lang=lang).search(records_domain)
    ...
    @staticmethod
    def X_generate_csv_data(labels, rows):
        fp = io.BytesIO()
        ...
        for data in rows:
            ...
            if labels.index('ID') :
                modified_data[labels.index('ID')] =  modified_data[labels.index('ID')][1]  
            modified_rows.append(modified_data)
        
        for modified_data in modified_rows:

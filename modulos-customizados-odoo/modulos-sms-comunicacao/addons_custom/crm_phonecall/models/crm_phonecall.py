...
class CrmPhonecall(models.Model):
    ...
    date_action_last = fields.Datetime(string="Last Action", readonly=True, tracking=True)
    date_action_next = fields.Datetime(string="Next Action", readonly=True, tracking=True)
    create_date = fields.Datetime(string="Creation Date", readonly=True, tracking=True)
    ...
    email_from = fields.Char(string="Email", help="These people will receive email.", tracking=True)
    date_open = fields.Datetime(string="Opened", readonly=True, tracking=True)
    name = fields.Char(string="Call Summary", required=True, tracking=True)
    active = fields.Boolean(required=False, default=True, tracking=True)
    duration = fields.Float(help="Duration in minutes and seconds.", tracking=True)
    ...
    partner_mobile = fields.Char("Mobile", tracking=True)
    priority = fields.Selection(
        selection=[("0", "Low"), ("1", "Normal"), ("2", "High")],
        default="1",
        tracking=True
    )
    date_closed = fields.Datetime(string="Closed", readonly=True, tracking=True)
    date = fields.Datetime(default=lambda self: fields.Datetime.now(), tracking=True)
    opportunity_id = fields.Many2one(comodel_name="crm.lead", string="Lead/Opportunity", tracking=True)
    direction = fields.Selection(
        [("in", "In"), ("out", "Out")], default="out", required=True, tracking=True
    )

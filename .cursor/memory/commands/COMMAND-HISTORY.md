...
### Odoo - Reiniciar Corretamente (CRÍTICO!)

**Problema:** Odoo com erro 502 rodando em modo debug (--workers=0)
**Solução:** Reiniciar com workers de produção

```bash
# ✅ MÉTODO CORRETO - Systemd (já configurado com workers)
sudo systemctl restart odoo-server

# ✅ Verificar se tem múltiplos processos (> 5 = produção)
ps aux | grep '[o]doo-bin' | wc -l

# ✅ Se necessário, reiniciar Nginx também
...
**Regra aprendida:** `--workers=0` = debug (1 processo), `production` = 9+ processos
**Data:** 2025-11-19
**Contexto:** Erro 502 Bad Gateway - modo debug causa lentidão
**Trigger:** Quando Odoo estiver lento ou retornando 502

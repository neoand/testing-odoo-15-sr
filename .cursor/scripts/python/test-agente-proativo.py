...
def test_basic_functionality():
    ...
    # Os arquivos estão no mesmo diretório do script
    ...
    modules['context'] = test_import_module(
        'ContextAnalysisEngine', str(script_dir / 'agent-proativo-core.py')
    )

    modules['refinement'] = test_import_module(
        'RefinementEngine', str(script_dir / 'refinement-engine.py')
    )

    modules['suggestions'] = test_import_module(
        'SuggestionsEngine', str(script_dir / 'suggestions-engine.py')
    )

    modules['pattern'] = test_import_module(
        'PatternDetector', str(script_dir / 'pattern-detector.py')
    )

    modules['learning'] = test_import_module(
        'LearningLoop', str(script_dir / 'learning-loop.py')
    )
    ...
    if carregados == total:
        ...
        if modules['context'] and modules['refinement']:
            try:
                ...
                ContextAnalysisEngine = modules['context']
                project_root = script_dir.parent.parent.parent  # Volta para raiz do projeto
...
def test_simple_scenario():
    ...
    context_module = test_import_module('ContextAnalysisEngine', str(script_dir / 'agent-proativo-core.py'))
    refinement_module = test_import_module('RefinementEngine', str(script_dir / 'refinement-engine.py'))
    suggestions_module = test_import_module('SuggestionsEngine', str(script_dir / 'suggestions-engine.py'))

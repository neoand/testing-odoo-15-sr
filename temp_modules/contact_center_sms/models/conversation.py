...
class AcruxChatConversation(models.Model):
    ...
    # Informações específicas de SMS
    sms_segment_count = fields.Integer('SMS Segments', compute='_compute_sms_info', store=False,
                                       help='Number of SMS segments in last message')
    sms_cost = fields.Float('SMS Cost', compute='_compute_sms_info', store=False,
                           help='Estimated cost of last SMS sent')

    @api.depends('last_message_text', 'channel_type')
    def _compute_sms_info(self):
        """Calcula segmentos e custo estimado para SMS"""
        for conv in self:
            if conv.channel_type == 'sms' and conv.last_message_text:
                text_len = len(conv.last_message_text)
                # SMS: 160 chars por segmento (padrão), 70 se tiver unicode
                has_unicode = any(ord(c) > 127 for c in conv.last_message_text)
                segment_size = 70 if has_unicode else 160
                conv.sms_segment_count = (text_len // segment_size) + (1 if text_len % segment_size else 0)
                # Custo aproximado: R$ 0,10 por segmento
                conv.sms_cost = conv.sms_segment_count * 0.10
            else:
                conv.sms_segment_count = 0
                conv.sms_cost = 0.0

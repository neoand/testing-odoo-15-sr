...
class SMSComposer(models.TransientModel):
    ...
    def _onchange_template_id(self):
        ...
        if self.template_id and self.partner_ids:
            ...
            try:
                # Use Odoo's template rendering with proper context
                rendered = self.template_id._render_template(
                    self.template_id.body,
                    self.template_id.model,
                    [partner.id],
                    post_process=True
                )
                self.body = rendered.get(partner.id, self.template_id.body)
            except Exception as e:
                _logger.warning(f"Error rendering template: {e}")
                # Fallback: just use template body
        ...
        for phone_data in phones:
            ...
            if self.template_id:
                try:
                    ...
                    body = rendered.get(partner.id, self.body)
                    _logger.info(f"Rendered SMS for {partner.name}: {body[:100]}")
                except Exception as e:
                    _logger.error(f"Error rendering template for {partner.name}: {e}", exc_info=True)
                    # If template rendering fails, use body as-is
                    body = self.body
